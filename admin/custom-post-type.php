...
// Register Custom Post Type
function create_booking_return_post_type(){
    register_post_type('booking-return',
        array(
            'labels' => array(
                'name' => __('Hour Bookings Return'),
                'singular_name' => __('Booking Return')
            ),
            'public' => true,
            'has_archive' => true,
            'menu_icon' => 'dashicons-calendar-alt',
...
add_action('init', 'create_booking_return_post_type');
...
// Register Custom Post Type
function create_saver_booking_return_post_type() {
    register_post_type('saver-booking-return',
        array(
            'labels' => array(
                'name' => __('Saver Bookings Return'),
                'singular_name' => __('Saver Booking Return')
            ),
            'public' => true,
            'has_archive' => true,
            'menu_icon' => 'dashicons-calendar-alt',
...
add_action('init', 'create_saver_booking_return_post_type');
...
// Register Custom Post Type collection return
function create_collection_return_post_type() {
    register_post_type('collection-return',
        array(
            'labels' => array(
                'name' => __('Collection Bookings Return'),
                'singular_name' => __('Collection Booking Return')
            ),
            'public' => true,
            'has_archive' => true,
            'menu_icon' => 'dashicons-calendar-alt',
...
add_action('init', 'create_collection_return_post_type');
...
// add meta box for custom fields in booking return post type
function add_booking_return_meta_boxes(){
    add_meta_box(
        'booking_return_details_meta_box', // ID
        'Hour Booking Return Details', // Title
        'display_booking_return_meta_box', // Callback
        'booking-return', // Post type
        'normal', // Context
        'high' // Priority
    );
}
add_action('add_meta_boxes', 'add_booking_return_meta_boxes');
...
// add meta box for custom fields in saver return post type
function add_saver_return_booking_meta_boxes(){
    add_meta_box(
        'saver_booking_return_details_meta_box', // ID
        'Saver Booking Return Details', // Title
        'display_saver_booking_return_meta_box', // Callback
        'saver-booking-return', // Post type
        'normal', // Context
        'high' // Priority
    );
}
add_action('add_meta_boxes', 'add_saver_return_booking_meta_boxes');
...
// add meta box for custom fields in collection return post type
function add_collection_return_meta_boxes(){
    add_meta_box(
        'collection_return_details_meta_box', // ID
        'Collection Return Details', // Title
        'display_collection_return_meta_box', // Callback
        'collection-return', // Post type
        'normal', // Context
        'high' // Priority
    );
}
add_action('add_meta_boxes', 'add_collection_return_meta_boxes');
...
// display booking return meta box
function display_booking_return_meta_box($post){
    $date = get_post_meta($post->ID, '_booking_return_date', true);
    $time_slot = get_post_meta($post->ID, '_booking_return_time_slot', true);
    $status = get_post_meta($post->ID, '_booking_return_status', true);
    $price = get_post_meta($post->ID, '_booking_return_price', true);
    ?>
    <div class="booking-form">
        <!-- Date Field -->
        <label for="booking_return_date">Date:</label>
        <input type="date" name="booking_return_date" id="booking_return_date" value="<?php echo esc_attr($date); ?>">

        <!-- Time Slot Field -->
        <label for="booking_return_time_slot">Time Slot:</label>
        <select name="booking_return_time_slot" id="booking_return_time_slot">
            <option value="6am - 7am" <?php selected($time_slot, '6am - 7am'); ?>>6am - 7am</option>
            <option value="7am - 8am" <?php selected($time_slot, '7am - 8am'); ?>>7am - 8am</option>
            <option value="8am - 9am" <?php selected($time_slot, '8am - 9am'); ?>>8am - 9am</option>
            <option value="9am - 10am" <?php selected($time_slot, '9am - 10am'); ?>>9am - 10am</option>
        ...
        <!-- Status Field -->
        <label for="booking_return_status">Status:</label>
        <select name="booking_return_status" id="booking_return_status">
            <option value="available" <?php selected($status, 'available'); ?>>Available</option>
            <option value="fully_booked" <?php selected($status, 'fully_booked'); ?>>Fully Booked</option>
            <option value="unavailable" <?php selected($status, 'unavailable'); ?>>Unavailable</option>
        </select>
        ...
        <!-- Price Field -->
        <label for="booking_return_price">Price:</label>
        <input type="text" name="booking_return_price" id="booking_return_price" placeholder="£" value="<?php echo esc_attr($price); ?>">
    </div>
    <?php
}
...
// saver return display booking meta box
function display_saver_booking_return_meta_box($post){
    $date = get_post_meta($post->ID, '_saver_booking_return_date', true);
    $time_slot = get_post_meta($post->ID, '_saver_booking_return_time_slot', true);
    $status = get_post_meta($post->ID, '_saver_booking_return_status', true);
    $price = get_post_meta($post->ID, '_saver_booking_return_price', true);
    ?>
    <div class="booking-form">
        <!-- Date Field -->
        <label for="saver_booking_return_date">Date:</label>
        <input type="date" name="saver_booking_return_date" id="saver_booking_return_date" value="<?php echo esc_attr($date); ?>">

        <!-- Time Slot Field -->
        <label for="saver_booking_return_time_slot">Time Slot:</label>
        <select name="saver_booking_return_time_slot" id="saver_booking_return_time_slot">
            <option value="8am - 12pm" <?php selected($time_slot, '8am - 12pm'); ?>>8am - 12pm</option>
            <option value="12pm - 4pm" <?php selected($time_slot, '12pm - 4pm'); ?>>12pm - 4pm</option>
            <option value="4pm - 8pm" <?php selected($time_slot, '4pm - 8pm'); ?>>4pm - 8pm</option>
        </select>
        ...
        <!-- Status Field -->
        <label for="saver_booking_return_status">Status:</label>
        <select name="saver_booking_return_status" id="saver_booking_return_status">
            <option value="available" <?php selected($status, 'available'); ?>>Available</option>
            <option value="fully_booked" <?php selected($status, 'fully_booked'); ?>>Fully Booked</option>
            <option value="unavailable" <?php selected($status, 'unavailable'); ?>>Unavailable</option>
        </select>
        ...
        <!-- Price Field -->
        <label for="saver_booking_return_price">Price:</label>
        <input type="text" name="saver_booking_return_price" id="saver_booking_return_price" placeholder="£" value="<?php echo esc_attr($price); ?>">
    </div>
...
// collection return display booking meta box
function display_collection_return_meta_box($post){
    $date = get_post_meta($post->ID, '_collection_booking_return_date', true);
    $time_slot = get_post_meta($post->ID, '_collection_booking_return_time_slot', true);
    $status = get_post_meta($post->ID, '_collection_booking_return_status', true);
    $price = get_post_meta($post->ID, '_collection_booking_return_price', true);
    ?>
        <div class="booking-form">
            <!-- Date Field -->
            <label for="collection_booking_return_date">Date:</label>
            <input type="date" name="collection_booking_return_date" id="collection_booking_return_date" value="<?php echo esc_attr($date); ?>">

            <!-- Time Slot Field -->
            <label for="collection_booking_return_time_slot">Time Slot:</label>
            <select name="collection_booking_return_time_slot" id="collection_booking_return_time_slot">
                <option value="10am - 11am" <?php selected($time_slot, '10am - 11am'); ?>>10am - 11am</option>
                <option value="11am - 12pm" <?php selected($time_slot, '11am - 12pm'); ?>>11am - 12pm</option>
                <option value="12pm - 1pm" <?php selected($time_slot, '12pm - 1pm'); ?>>12pm - 1pm</option>
                <option value="1pm - 2pm" <?php selected($time_slot, '1pm - 2pm'); ?>>1pm - 2pm</option>
            ...
            <!-- Status Field -->
            <label for="collection_booking_return_status">Status:</label>
            <select name="collection_booking_return_status" id="collection_booking_return_status">
                <option value="available" <?php selected($status, 'available'); ?>>Available</option>
                <option value="fully_booked" <?php selected($status, 'fully_booked'); ?>>Fully Booked</option>
                <option value="unavailable" <?php selected($status, 'unavailable'); ?>>Unavailable</option>
            </select>
            ...
            <!-- Price Field -->
            <label for="collection_booking_return_price">Price:</label>
            <input type="text" name="collection_booking_return_price" id="collection_booking_return_price" placeholder="£" value="<?php echo esc_attr($price); ?>">
        </div>
    <?php
}
...
// save booking return meta box
function save_booking_return_meta_box_data($post_id) {
    // Verify this is not an auto-save routine.
    if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) return;
    ...
    // Save the meta box data as post meta
    if (isset($_POST['booking_return_date'])) {
        update_post_meta($post_id, '_booking_return_date', sanitize_text_field($_POST['booking_return_date']));
    }
    if (isset($_POST['booking_return_time_slot'])) {
        update_post_meta($post_id, '_booking_return_time_slot', sanitize_text_field($_POST['booking_return_time_slot']));
    }
    if (isset($_POST['booking_return_status'])) {
        update_post_meta($post_id, '_booking_return_status', sanitize_text_field($_POST['booking_return_status']));
    }
    if (isset($_POST['booking_return_price'])) {
        update_post_meta($post_id, '_booking_return_price', sanitize_text_field($_POST['booking_return_price']));
    }
}
add_action('save_post', 'save_booking_return_meta_box_data');
...
// collection save booking return meta box
function save_collection_booking_return_meta_box_data($post_id) {
    // Verify this is not an auto-save routine.
    if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) return;
    ...
    // Save the meta box data as post meta
    if (isset($_POST['collection_booking_return_date'])) {
        update_post_meta($post_id, '_collection_booking_return_date', sanitize_text_field($_POST['collection_booking_return_date']));
    }
    if (isset($_POST['collection_booking_return_time_slot'])) {
        update_post_meta($post_id, '_collection_booking_return_time_slot', sanitize_text_field($_POST['collection_booking_return_time_slot']));
    }
    if (isset($_POST['collection_booking_return_status'])) {
        update_post_meta($post_id, '_collection_booking_return_status', sanitize_text_field($_POST['collection_booking_return_status']));
    }
    if (isset($_POST['collection_booking_return_price'])) {
        update_post_meta($post_id, '_collection_booking_return_price', sanitize_text_field($_POST['collection_booking_return_price']));
    }
}
add_action('save_post', 'save_collection_booking_return_meta_box_data');
